...
const HandDetection = ({
  ...
  const handleStartNotePlay = useCallback(
    (note: string, finger: string, hand: string) => {
      if (piano.current) {
  ...
  const previousFingerPositions = useRef<{
  ...
  const drawResults = (hands: handPoseDetection.Hand[]) => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (
        notesBeingPlayed.current.some(
          (playedNote) => playedNote.note.note === note
        )
      ) {
    ...
    hands.forEach((hand, handIndex) => {
      ...
      if (!previousFingerPositions.current[handId]) {
        previousFingerPositions.current[handId] = hand.keypoints.map((kp) => ({
          ...
          y: kp.y,
          z: hand.keypoints3D
            ? hand.keypoints3D[hand.keypoints.indexOf(kp)].z || 0
            : 0,
        }));
        return;
      }
      ...
      hand.keypoints.forEach((keypoint, i) => {
        ...
        const smoothedX =
          prev.x * smoothingFactor + keypoint.x * (1 - smoothingFactor);
        const smoothedY =
          prev.y * smoothingFactor + keypoint.y * (1 - smoothingFactor);
        ...
        if (hand.keypoints3D && hand.keypoints3D[i]) {
          ...
          const smoothedZ =
            prev.z * smoothingFactor + zValue * (1 - smoothingFactor);
          if (
            smoothedHands[handIndex].keypoints3D &&
            smoothedHands[handIndex].keypoints3D[i]
          ) {
  ...
  const detectHands = async () => {
    ...
    try {
      // eslint-disable-next-line
      let hands = await (window as any).handDetector.estimateHands(
        videoRef.current
      );
  ...
  const checkNotes = (hands: handPoseDetection.Hand[]) => {
    ...
    const playedNotes = fingertips.flatMap((finger) => {
      ...
      const hotspot = hotspots.find(
        (spot: { x: number; y: number; note: string }) => {
    ...
    if (playedNotes.length > 0) {
      console.log(
        "Notes being played:",
        playedNotes.map((note) => note.note.note).join(", ")
      );
    }
    ...
    if (fingertips.length > 0 && hotspots.length > 0) {
      console.log(
        `Found ${fingertips.length} fingertips and ${hotspots.length} hotspots`
      );
    }
    ...
    for (const note of notesBeingPlayed.current) {
      if (
        playedNotes.every(
          (playedNote) => playedNote.note.note !== note.note.note
        )
      ) {
        onEndNotePlay(
          note.note.note,
          note.finger!,
          note.hand === "Left" ? "Right" : "Left"
        );
      }
    }
    ...
    for (const playedNote of playedNotes) {
      if (
        !notesBeingPlayed.current.some(
          (noteBeingPlayed) =>
            noteBeingPlayed.note.note === playedNote.note.note
        )
      ) {
        handleStartNotePlay(
          playedNote.note.note,
          playedNote.finger!,
          playedNote.hand === "Left" ? "Right" : "Left"
        );
      }
    }
